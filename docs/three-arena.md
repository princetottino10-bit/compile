...
**効果種別エフェクト** — カードの動きに色を添える軽量演出 (`fx.js`):
draw=光の尾 / flip=閃光リング / shift=残像の帯 / delete=赤い粒子の飛散。
`board.js` の差分移動が種別を判定して呼ぶ。

**決めのカメラ** (`stage.cinematicHold`) — コンパイルと決着で、通常の俯瞰から
レーンへ低く回り込む。演出時間ぶんカメラを占有し、終わると `stage.home` で戻る。
...
**アンビエントBGM** — 低音ドローン + うねるフィルタ + まばらな高音のきらめきを
合成 (`startBgm` / `setBgmTension`)。盤面の切迫度 (最大ライン合計・コンパイル数) で
緊張度 0..1 が上下し、明るさと厚みが増す。最初の操作で自動開始、🔊 で停止/再開。
