...
### B. 常時起動の機器が無い場合 (iPhone と PC だけ)

App Store の Wake-on-LAN アプリを使います。Siri・ショートカット・ウィジェットに
対応しているものが複数あります。

| アプリ | 備考 |
| --- | --- |
| [Wake Me Up - Wake-on-LAN](https://apps.apple.com/us/app/wake-me-up-wake-on-lan/id1465416032) | Siri とショートカットに対応 |
| [Magic Packet: WOL](https://apps.apple.com/us/app/magic-packet-wol-wake-on-lan/id1488937601) | Siri・ウィジェット・Apple Watch・コントロールセンター |
| [WakeyToo](https://apps.apple.com/us/app/wakeytoo/id6648755799) | 無料。ショートカットと Siri に対応 |

アプリに入れる値は3つです。

- **MAC アドレス** — `wake_pc_setup.ps1` が最後に表示したもの
- **ブロードキャストアドレス** — PC の IP が `192.168.1.20` なら `192.168.1.255`
  (第4オクテットを 255 にする)
- **ポート** — `9`

Siri から呼びたい場合は、アプリが作るショートカットの名前を「PC起こして」など
言いやすいものにしておきます。

この構成には制約があります。

- **iPhone が自宅の Wi-Fi に繋がっているときしか起こせません。** モバイル回線からは
  マジックパケットが届きません。
- 外出先から起こしたいなら、ルータの VPN サーバ機能 (WireGuard / L2TP) を有効にして
  VPN 経由で繋ぐか、ルータに「インターネットからの Wake on LAN」機能があれば
  それを使います。「[外出先から起こす](#外出先から起こす)」も参照。

この場合 `wake_pc.py` は出番がありませんが、**起こされる側のPCの設定
(`wake_pc_setup.ps1` と BIOS) はそのまま必要**です。
...
iPhone と PC しか無い場合は、ルータの VPN サーバ機能が使えるかどうかで決まります。
使えないなら、外出先から起こすのは諦めて自宅の Wi-Fi 内だけで使うか、
常時起動の機器 (Raspberry Pi など) を1台足すことになります。
