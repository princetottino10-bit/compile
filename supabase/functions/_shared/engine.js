...
function log(ctx, msg, uid) {
  ...
  /* chain: いま処理中の効果の並び (外側 → 内側)。効果の途中で別の効果が割り込んだとき
     (FIRE 0 の「覆われるとき」等) に、クライアントがチェーン表示に使う */
  if (TRACE && ctx.trace) {
    ctx.trace.push({ msg, uid: uid || null, chain: ctx.chain ? ctx.chain.slice() : [], st: clone(ctx.st) });
  }
}
...
/* (M2ヘルパー群) */
/* 裏向きのカードにはプロトコルが無い (値2の札として扱う)。
   プロトコルの種類・枚数を数える効果 (DIVERSITY / UNITY) は表向きのカードだけを数える */
/* フィールドにあるコマンドカードのプロトコル種類数 (DIVERSITY) */
function countProtoKinds(st) {
  const kinds = new Set();
  for (let l = 0; l < 3; l++) for (let s = 0; s < 2; s++)
    for (const uid of st.lines[l][s]) if (st.cards[uid].faceUp) kinds.add(DEFS[st.cards[uid].def].proto);
  return kinds.size;
}
...
function countProtoOnField(st, proto, excludeUid) {
  ...
  for (let l = 0; l < 3; l++) for (let s = 0; s < 2; s++)
    for (const uid of st.lines[l][s])
      if (uid !== excludeUid && st.cards[uid].faceUp && DEFS[st.cards[uid].def].proto === proto) n++;
  return n;
}
/* ライン内のプロトコル種類数 (DIVERSITY_2) */
function countProtoKindsInLine(st, line) {
  const kinds = new Set();
  for (let s = 0; s < 2; s++) for (const uid of st.lines[line][s]) if (st.cards[uid].faceUp) kinds.add(DEFS[st.cards[uid].def].proto);
  return kinds.size;
}
...
function execOp(ctx, fr, op) {
  ...
  switch (op.op) {
    ...
    case 'setProtocolCompiled': { // DIVERSITY_1 / UNITY_2: 自分の該当プロトコルをコンパイル完了にする
      ...
      if (hasCompiledToWin(st, fr.controller)) {
...
const AI_W = {
  ...
  refreshUrgency: 120,
  /* 選択の答えを実際に試す深さ。1 なら「選択の中の選択」まで試す。
     GRAVITY の連鎖 (反転で GRAVITY 1 を表に → その中段の移動で GRAVITY 2 を上に重ねる
     → 残りの移動で GRAVITY 1 の中段がもう一度入り、計4枚引く) は2段目の選択で決まる */
  choiceDepth: 1,
  /* 解決順の選択 (同時に誘発した効果をどれから解くか) も、候補がこの数以下なら
     実際に解決して盤面で比べる。0 なら従来どおり固定の順位で答える */
  orderSim: 4,
  /* DIVERSITY 0 の構え (aiDiversityReady): 手札に DIVERSITY 0 があり、場に DIVERSITY 以外が5種類そろっていれば
     次の手番に出すだけでコンパイルできる。1手先の読みでは見えないので、そろえる手のために置く */
  diversityReady: 120,
  /* DIVERSITY 0 は10点を積まずに1本通す切り札。3本目 (最後) に使えば、相手が点数のラインを止めていても勝てる。
     diversityLast: 他の2本が済んだあとの構えの価値 / diversityEarly: 1本目・2本目で使ってしまう減点 */
...
function aiControlLeverage(st, side) {
  ...
  const op = 1 - side;
  const myComp = aiCompiledTowardWin(st, side);
  const opComp = aiCompiledTowardWin(st, op);
...
/* ロック特化: スピード③の終了時の移動 (任意。使うとスピード③が裏返る) を見送るか。
   ①を覆う前にこれを使うと、覆う道具 (表のスピード③) を失い、値も 3 → 2 になる。
   ロックが未完成で、①が手札にあり、いま覆える表の①が場に無いなら見送る。
   表の①が一番上にあるときは、移動で覆えばロックが完成するので通常の評価に任せる */
function aiLockKeepsCover(st, side) {
  if (!aiIsLockSpecialist(st, side)) return false;
  const lockLive = activeStatics(st).some(s => s.rule === 'oppFaceDownOnly' && s.sideIdx === side
    && st.lines[s.line][s.sideIdx].indexOf(s.uid) < st.lines[s.line][s.sideIdx].length - 1);
  if (lockLive) return false;
  let coverable = false, lockOnField = false;
  for (let l = 0; l < 3; l++) {
    const stack = st.lines[l][side];
    stack.forEach((uid, i) => {
      if (st.cards[uid].def !== AI_LOCK_CARD) return;
      lockOnField = true;
      if (st.cards[uid].faceUp && i === stack.length - 1) coverable = true;
    });
  }
  if (coverable) return false;
  /* 次の手番に①を表で出して覆うルートが本当に使える (①が手札にある) ときだけ残す。
     山札にあるだけの段階で温存すると、裏向きのスピード③をダークネスのラインで値4にする
     仕込みを逃して、ミラー400戦で勝率 45% に落ちた */
  return aiHasDefInHand(st, side, AI_LOCK_CARD);
}
...
/* DIVERSITY 0 (docs/ai-combos.md): 手札の DIVERSITY 0 を表で出せて、自分の DIVERSITY が未コンパイルで、
   場に DIVERSITY 以外のプロトコルが表向きで5種類あれば、次の手番に出すだけでコンパイルできる */
function aiDiversityReady(st, side) {
  ...
  for (let l = 0; l < 3; l++) for (let s = 0; s < 2; s++) for (const uid of st.lines[l][s]) {
    if (st.cards[uid].faceUp) kinds.add(DEFS[st.cards[uid].def].proto);
  }
  kinds.delete('DIVERSITY');
  if (kinds.size < 5) return false;
  return [0, 1, 2].some(line => canPlay(st, side, d0, line, true));
}
...
function aiScore(st, me) {
  ...
  let sc = 0;
  const myComp = aiCompiledTowardWin(st, me);
  const opComp = aiCompiledTowardWin(st, op);
  ...
  const myGaps = [], opGaps = [];
  /* DIVERSITY 0 を切り札として持っている間は、DIVERSITY のラインを10点で普通にコンパイルすると切り札が無駄になる */
  const saveDiversity = W.diversityHoldLine && aiHoldsDiversityTrump(st, me);
  for (let l = 0; l < 3; l++) {
    const li = lineInfo[l];
...
function aiPlayFreePicks(st, req, me) {
  ...
  if (AI_CHOICE_DEPTH > aiWeightsFor(st, me).choiceDepth) return [ranked[0].raw];

  let best = ranked[0].raw, bestScore = -Infinity;
  /* 深い段ほど候補を絞る (試す数が段ごとに掛け算で増えるため) */
  for (const item of ranked.slice(0, AI_CHOICE_DEPTH === 0 ? 6 : 3)) {
    if (aiSearchExpired()) break;
...
function aiStrategicCardPicks(st, req, me, ranked, fallback) {
  ...
  if (AI_CHOICE_DEPTH > W.choiceDepth || choices < 2) return fallback;
  /* 解決順は候補が増えやすい (誘発が3つ4つ同時に乗る)。少ないときだけ試す */
  if (/(?:^|-)order$/.test(req.prompt || '') && (!W.orderSim || req.candidates.length > W.orderSim)) {
    return fallback;
  }
  if (max !== 1) {
  ...
  /* 深い段ほど候補を絞る。静的評価の上位と下位を残すのは、
     下位 (自分のカードを動かす等) に連鎖の起点があることが多いため */
  const width = AI_CHOICE_DEPTH === 0 ? 8 : 5;
  let pool = ranked;
  if (pool.length > width) pool = pool.slice(0, width - 3).concat(pool.slice(-3));
...
function smartPicks(st, req) {
  ...
  switch (req.kind) {
    ...
    case 'pickHand': {
      ...
      /* 「捨ててもよい」は、捨てない + 候補ごとに捨てた盤面で比べる。以前は一番値の低い札しか試さず、
         組み合わせで使う値0の札 (DIVERSITY 0 / SPEED 0 など) を真っ先に捨てていた */
      if (min === 0 && max === 1 && scored.length && AI_CHOICE_DEPTH === 0) {
        return aiStrategicCardPicks(st, req, me, scored, fallbackHand);
      }
      /* 1枚必須 (捨てる・渡す等) も、選んだあとの盤面で比べる。値の低い札から機械的に選ぶと、
         組み合わせで使う札 (SPEED 0 → SPEED 3 の 0 など) を真っ先に捨てていた */
      if (max === 1 && min >= 1 && scored.length > 1) {
        return aiStrategicCardPicks(st, req, me, scored, fallbackHand);
      }
    ...
    case 'pickLine': {
      ...
      if (AI_CHOICE_DEPTH <= aiWeightsFor(st, me).choiceDepth && req.lines.length > 1) {
...
function aiDecisionActions(state) {
  ...
  const opponent = 1 - side;
  const opponentPending = 3 - aiCompiledTowardWin(state, opponent);   // 勝ちまであと何本
...
function aiActionHard(state, collect) {
  ...
  try {
    ...
    if (aiCompiledTowardWin(state, me) === 2) {
    ...
    if (aiCompiledTowardWin(state, 1 - me) === 2 && viable.length > 1) {
...
function minimaxMin(state, me, alpha, beta, deadline) {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    const s2 = res.state;
    let sc;
    if (s2.winner === null && s2.turn === me) {
      /* 自分の次の手まで読んだ値は、相手の手番を1つ挟んだ先の話。
         満額で数えるとリフレッシュのような「溜める手」が強く見えすぎる */
      const base = aiScore(s2, me);
      const deep = minimaxMaxShallow(s2, me, alpha, beta, deadline);
      sc = base + (deep - base) * aiWeightsFor(s2, me).futureDiscount;
    } else sc = aiScore(s2, me);
